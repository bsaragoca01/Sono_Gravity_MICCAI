...
#Model's hyperparameters. Must be changed at the end of the script
def train_model(
        model,
        device,
        epochs: int = 100, 
    ...
    #Create dataset: different ones for the training set and validation set
    dataset = CarvanaDataset(dir_img, dir_mask, img_scale, transform = None) #training set
    datasetTest = CarvanaDataset(dir_imgT, dir_maskT, img_scale, transform = None) #validation set
    ...
    for epoch in range(1, epochs + 1):
        ...
        scheduler.step(val_metrics['avg_accuracy']) #applies the learning rate 5 epoch tolerance to the accuracy
        #saves the checkpoint every 5 epochs
        if epoch%5==0:
