...
def dilate_mask(mask, pixels_per_mm=3.78, dilation_mm=1.5): 
...
def erode_mask(mask, pixels_per_mm=3.78, erosion_mm=1.5):
...
def evaluate_per_image(net, image, mask_true, device, amp=False, apply_margin=True):
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        ...
        if net.n_classes == 1:
            ...
            #Dice coefficient calculation with the original ground-truth masks:
            ...
            if apply_margin:
                ...
                # Calculates the dice using the dilated and eroded masks and select the best option for the current image:
            ...
            mask_pred_class_0 = mask_pred_one_hot[:, 0]  # Extracts class 0 prediction
            mask_pred_class_1 = mask_pred_one_hot[:, 1]  # Extracts class 1 prediction
            mask_pred_class_2 = mask_pred_one_hot[:, 2]  # Extracts class 2 prediction
            ...
            if apply_margin:
                eroded_mask_one_hot = F.one_hot(eroded_mask, net.n_classes).permute(0, 3, 1, 2).float()
                eroded_dice = multiclass_dice_coeff(mask_pred_one_hot, eroded_mask_one_hot)
                dilated_mask_one_hot = F.one_hot(dilated_mask, net.n_classes).permute(0, 3, 1, 2).float()
                dilated_dice = multiclass_dice_coeff(mask_pred_one_hot, dilated_mask_one_hot)

                if eroded_dice > initial_dice and eroded_dice > dilated_dice:
                    mask_true_one_hot = eroded_mask_one_hot
                    dice = eroded_dice
                    count_eroded_dice += 1 #count the number of ground-truth masks which were transformed to eroded
                    # Recalculate Dice per class using eroded mask
                    ...
                    mask_true_one_hot = dilated_mask_one_hot
                    dice = dilated_dice
                    count_dilated_dice += 1
                    dice_class_1 = dice_coeff(mask_pred_one_hot[:, 1], mask_true_one_hot[:,1], reduce_batch_first=False)
                    dice_class_2 = dice_coeff(mask_pred_one_hot[:,2], mask_true_one_hot[:,2], reduce_batch_first=False)
                else:
                    count_normal_dice += 1
