...
def apply_custom_color_mapping(mask, class_colors):
    ...
    for class_idx, color in class_colors.items():
        colored_mask[mask == class_idx] = color  #Applying color based on prediction class
    return colored_mask
...
def load_model():
    ...
    #model = segmentation.deeplabv3_resnet101(pretrained=False) if DeepLab with ResNet101 is intended to be used
...
def process_frame(frame_gray, original_frame):  #predict the images
    ...
    with torch.no_grad(), torch.cuda.amp.autocast():
        ...
        pred_mask = output.argmax(dim=1).squeeze().float()

    pred_mask_np = pred_mask.cpu().numpy().astype(np.uint8)
    colored_overlay = apply_custom_color_mapping(pred_mask_np, class_colors)
    #Resize colored overlay to original frame size
